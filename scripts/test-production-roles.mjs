...
console.log(`::notice title=G security passed::G1=${authHardening[1]}; G1b=${refreshRotation[1]}; G1c=${invitations[1]}; G1d=${totpManagement[1]}; G2=${rlsIntegrity[1]}; G3=${dpiaApproval[1]}; G4=${principalRevocation[1]}. All scenario thresholds verified with zero failures. G1 HTTP/proxy/PG, G1b atomic refresh rotation, G1c scoped invitations, G1d TOTP management, G2 application-role SQL, G3 independent DPIA approval/atomic audit and G4 global in-flight JWT revocation (epoch revalidated at entry guards, DB triggers for direct SQL); remaining limits per runbooks (TOTP-only changes, single-request guard-to-commit window); not privileged-helper authorization or production deployment qualification.`);
console.log('✓ GATE D : régressions Phase D + 56 suites/contrôles (E1–E6 incluses) avec rôles et grants de production.');
