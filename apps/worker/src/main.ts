...
async function drainNotificationQueue(): Promise<void> {
 ...
 try { await client.query('BEGIN'); const claimed=await client.query(`SELECT id,organization_id,user_id,title_fr,title_ar,body_fr,body_ar,data FROM notification_queue WHERE status='pending' AND scheduled_at<=NOW() ORDER BY created_at LIMIT 25 FOR UPDATE SKIP LOCKED`);
  for(const n of claimed.rows) {
   ...
   const devices=await client.query(`SELECT fcm_token FROM devices WHERE organization_id=$1 AND registered_by=$2 AND is_active=true AND revoked_at IS NULL AND fcm_token IS NOT NULL`,[n.organization_id,n.user_id]);
   try {
    if(devices.rows.length && googleAuth) for(const d of devices.rows) await fcmSend(d.fcm_token,{title:n.title_fr,body:n.body_fr,data:Object.fromEntries(Object.entries(n.data??{}).map(([k,v])=>[k,String(v)]))});
    // Sans jeton/configuration, l'inbox reste la voie fiable ; ne jamais envoyer de données sensibles à FCM.
    await client.query(`UPDATE notification_queue SET status='sent',sent_at=NOW(),failure_reason=CASE WHEN $2 THEN NULL ELSE 'FCM_NOT_CONFIGURED_OR_NO_DEVICE' END WHERE id=$1`,[n.id,Boolean(devices.rows.length && googleAuth)]);
