...
function PilotTab(): React.JSX.Element {
  ...
  const load = (): void => {
    api('GET', '/support/pilot-summary').then((r) => setRows(r as PilotRow[])).catch((e: unknown) => setError(e instanceof Error ? e.message : String(e)));
  };
  useEffect(load, []);
...
function FlagsTab(): React.JSX.Element {
  ...
  const load = (): void => {
    api('GET', '/support/flags').then((r) => setFlags(r as FlagRow[])).catch((e: unknown) => setError(e instanceof Error ? e.message : String(e)));
  };
  useEffect(load, []);
...
function JobsTab(): React.JSX.Element {
  ...
  const load = (): void => {
    api('GET', '/support/jobs').then((r) => setJobs(r as JobRow[])).catch((e: unknown) => setError(e instanceof Error ? e.message : String(e)));
  };
  useEffect(load, []);
