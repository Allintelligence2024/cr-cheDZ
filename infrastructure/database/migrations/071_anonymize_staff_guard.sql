-- ============================================================================
-- 071_anonymize_staff_guard.sql
-- Remédiation R6 / F4 (2026-09-21) — garde « lien staff » sur anonymize_child.
--
-- Constat (F4) : 067_anonymize_child.sql anonymise les tuteurs exclusifs,
-- y compris leur COMPTE USERS (identité, email, mot de passe, MFA, sessions)
-- et désactive TOUS leurs memberships de l'organisation. Si ce tuteur est
-- aussi un employé du personnel de l'organisation (rôle principal staff OU
-- rôle additionnel via role_assignments — migration 040), l'anonymisation
-- détruit son compte professionnel : l'employé perd ses accès, son mot de
-- passe et sa MFA, sans aucun chemin de retour.
--
-- Correction : 067 est immuable (ADR-007, checksums) → la fonction est
-- REMPLACÉE ici (CREATE OR REPLACE) : corps identique + UNE garde fail-closed,
-- AVANT tout écrit : si l'un des tuteurs exclusifs (v_users) est un membre
-- du personnel ACTIF de l'organisation → RAISE ANONYMIZE_STAFF_LINKED.
-- L'opérateur détache d'abord l'affiliation tuteur (ou le poste) puis rejeu.
-- Aucun effacement partiel d'un compte partagé (rejeté par conception : un
-- mot de passe partagé effacé est irréversible).
--
-- Pas de changement de signature (uuid, uuid, text) : les appelants
-- (privacy.service, scripts/anonymize.sql, tests) sont inchangés. Miroir
-- applicatif : privacy.service.ts mappe ANONYMIZE_STAFF_LINKED → 409.
-- ============================================================================
...
BEGIN
  ...
  -- ── Garde F4 (remédiation R6, 2026-09-21) : un tuteur exclusif de cet
  --    enfant qui est AUSSI un membre du personnel ACTIF de l'organisation
  --    (rôle principal staff OU rôle additionnel — role_assignments, 040)
  --    ne peut pas être anonymisé : l'anonymisation effacerait son compte
  --    professionnel et désactiverait ses accès. Refus explicite AVANT tout
  --    écrit (fail-closed) ; détacher d'abord l'affiliation tuteur ou le poste.
  IF EXISTS (
    SELECT 1 FROM memberships m
    JOIN roles r ON r.id = m.role_id
    WHERE m.user_id = ANY(v_users) AND m.organization_id = v_tenant AND m.is_active
      AND r.slug NOT IN ('parent_primary', 'parent_secondary')
  ) OR EXISTS (
    SELECT 1 FROM role_assignments ra
    JOIN roles r ON r.id = ra.role_id
    WHERE ra.user_id = ANY(v_users) AND ra.organization_id = v_tenant
      AND r.slug NOT IN ('parent_primary', 'parent_secondary')
  ) THEN
    RAISE EXCEPTION 'ANONYMIZE_STAFF_LINKED: au moins un tuteur exclusif de cet enfant est un membre du personnel actif de l''organisation — détachez d''abord l''affiliation tuteur (ou le poste) avant anonymisation'
      USING ERRCODE = 'P0001';
  END IF;
...
COMMENT ON FUNCTION anonymize_child(uuid, uuid, text) IS
  'Loi 25-11 — effacement par anonymisation d''un enfant sorti (et de ses tuteurs exclusifs). Transactionnel, idempotent, audité. 071 (R6/F4) : refus ANONYMIZE_STAFF_LINKED si un tuteur exclusif est un membre du personnel actif (garde avant tout écrit). Les FK RESTRICT interdisent la suppression physique par conception (DB6).';
