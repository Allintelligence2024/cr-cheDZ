-- ============================================================================
-- 037_fix_support_pilot_summary.sql
-- Correction de support_pilot_summary() (migration 036) : sync_operations
-- n'a pas de colonne created_at (le timestamp est received_at).
  ...
  RETURN QUERY
    SELECT
      ...
      (SELECT COUNT(*)::int FROM sync_operations so
        WHERE so.organization_id = o.id AND so.received_at >= NOW() - INTERVAL '24 hours'),
