...
INSERT INTO processing_registry
  (organization_id, processing_name, purpose_fr, purpose_ar, legal_basis,
   ...
   security_measures, dpo_notes, is_active)
SELECT NULL, v.processing_name, v.purpose_fr, v.purpose_ar, v.legal_basis,
       ...
       v.security_measures, v.dpo_notes, true
FROM (VALUES
  ('Photos des enfants',
   ...
   'Traitement sensible : DPIA requise (loi 25-11, art. 38)'),
  ('Dossier de santé de l''enfant',
   ...
   'Traitement sensible : accès parents verrouillé par permission'),
  ...
  ('Paie du personnel',
   ...
   'DPIA recommandée (données de santé du personnel)')
) AS v(processing_name, purpose_fr, purpose_ar, legal_basis, data_categories,
        data_subjects, retention_days, third_parties, security_measures, dpo_notes)
WHERE NOT EXISTS (
