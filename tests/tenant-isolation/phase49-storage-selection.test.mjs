...
import pg from 'pg';
import { ensureAppRole, appUrl, PRODUCTION_SPAWN_ENV } from './helpers.mjs';
...
// `safe` : environnement de production complet — le raccourci de banc
// d'essai RATE_LIMIT_DISABLED est refusé au boot en production (garde lot 1),
// donc explicitement neutralisé (voir PRODUCTION_SPAWN_ENV).
const safe = { ...PRODUCTION_SPAWN_ENV, NODE_ENV: 'production', JWT_SECRET: 'synthetic-h2i-jwt-0123456789abcdef0123456789', PAYMENT_WEBHOOK_SECRET: 'synthetic-h2i-webhook-0123456789abcdef0123456789', TOTP_ENCRYPTION_KEY: 'c'.repeat(64), STORAGE_BACKEND: 's3', STORAGE_LOCAL_DIR: root, S3_ACCESS_KEY: 'synthetic-h2i-access', S3_SECRET_KEY: 'synthetic-h2i-secret', SENTRY_DSN: '', SATIM_MERCHANT_ID: '', SATIM_SECRET: '', SATIM_GATEWAY_URL: '', FIREBASE_SERVICE_ACCOUNT_JSON: '', WORKER_SCHEDULER_ENABLED: 'false', WORKER_SHUTDOWN_TIMEOUT_MS: '1500', APP_PORT: '0' };
