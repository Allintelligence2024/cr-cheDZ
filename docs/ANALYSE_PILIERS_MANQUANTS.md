...
| Pilier | État | Preuve |
|---|---|---|
| Multi-tenancy RLS + rôles prod NOBYPASSRLS | ✅ SOLIDE | **72 entrées** d'isolation (Gate D : 70 `phaseNN` + `schema-check` + `rls-behavior-check`), `scripts/test-production-roles.mjs` |
...
|---|---|---|---|---|
| P1-1 ✅ (2026-09-20, code) / ⏳ (sandbox SATIM = contrat marchand) | **Paiement en ligne effectif (SATIM CIB/Edahabia)** | Cœur du marché DZ. | **FAIT côté logiciel** — `GET /billing/payments/online/reconciliation` (journal de rapprochement : confirmés / en attente / **en souffrance** = pending sans webhook au-delà de `stale_minutes`, avec `transaction_id` PSP à réclamer / échoués ; totaux ; jamais de statut deviné) ; `scripts/satim-sandbox-server.mjs` (simulateur au contrat exact de l'adaptateur : init signé HMAC + page de paiement + rappel webhook signé) ; `scripts/satim-sandbox-proof.mjs` (scénario de recette rejouable **tel quel contre la sandbox SATIM réelle** : login → init réel → webhook → facture payée → rapprochement, échec explicite sinon) ; `docs/paiement/SATIM.md` (enrôlement marchand, URLs à fournir, exploitation des lignes en souffrance). Gate **phase60** : simulateur en processus réel, script de preuve exécuté, parcours « navigateur » (/pay → /confirm → webhook réel), rejeu idempotent, rapprochement, isolation. **Reste externe** : convention e-paiement + identifiants sandbox SATIM (aucune ligne de code ne dépend de leur format : mêmes variables `SATIM_*`). | **M** |
| P1-2 ✅ (2026-09-20) | **Applications mobiles publiablement compilées** | Tout le travail sync/OTP/médias était invérifiable sur appareil. | **FAIT** — `flutter.yml` compile réellement les deux apps (`flutter build apk --release`, JDK 17) et vérifie la présence des APK (artefacts `android-apk-*`). Signature : `build.gradle.kts` (staff + parent) lit `android/key.properties` ou `ANDROID_KEYSTORE_*` ; **sans keystore, le build release échoue explicitement** (jamais d'APK « release » signé debug par erreur) sauf autorisation explicite `allowDebugSigning` — utilisée par la CI uniquement quand les secrets `ANDROID_KEYSTORE_BASE64/_PASSWORD`, `ANDROID_KEY_ALIAS/_PASSWORD` sont absents (artefact nommé `debug-signed`). Avec les secrets : APK + **app bundles** signés (publiables), artefact `signed`. `key.properties.example` + `.gitignore` (keystores/mots de passe jamais commités). **Reste externe** : générer le keystore de production (commande dans l'exemple) et le compte Play/Apple (ipa non compilé : aucun compte Apple Developer). | **M** |
| P1-3 ✅ (2026-09-20) | **Tests de charge / capacité** | Aucun test de charge exécutable (le `sync.k6.js` n'a jamais tourné : k6 absent, aucun outil de charge dans le dépôt). | Scénario réaliste reproductible, budgets p95, exécution pré-release. | **S-M** — **FAIT** (`npm run test:capacity`, `tests/load/capacity-bench.mjs`, Node pur sans k6) : 12 structures simultanées × 15 enfants × 2 appareils sync, 456 requêtes (login en rafale, check-in, `POST /sync/push` 20 ops, feed, dashboard), rôle applicatif NOBYPASSRLS, refus hors base `*_test`, rapport `capacity-report.json`, nettoyage complet. **Mesure (2 vCPU, PG local) : 0 erreur, 1920/1920 événements persistés ; p95 login 2,3 s · check-in 0,1 s · sync push 2,3 s · feed 1,2 s · dashboard 1,2 s.** Budgets = cliquet (+25 %), pas objectifs produit. **Goulot identifié : `bcryptjs` cost 12 = ~320 ms CPU par comparaison, bloque l'event loop** — 24 logins concurrents gonflent toutes les latences (feed 7 ms seul → 1,1 s sous rafale ; avec `NO_BURST_LOGIN=1` : feed 200 ms, dashboard 214 ms, push 1,16 s). Pistes (hors lot, aucune baisse du coût en prod) : hash natif hors event loop (`argon2` mesuré 96 ms/verif et 24 concurrents en 2,3 s vs 7,5 s bcryptjs) ou worker thread ; `sync push` traite ses ops séquentiellement, chacune dans sa connexion tenant (`sync.service.ts`) → batcher dans une transaction. Reste à faire : exports xlsx concurrents et worker sous backlog non couverts. **Complément (25/09/2026)** : le critère écrit pour k6 (« p95 sync push < 2 s pour 500 ops ») a été **mesuré par ce banc en parité exacte** avec le scénario k6 (50 pushes × 10 ops = 500 ops, `ORGS=10 DEVICES=5 OPS=10 BURST_ROUNDS=0`) : p95 **1 406 ms**, 0 erreur, 500/500 événements persistés, tous budgets p95 tenus — le script `sync.k6.js` lui-même reste **non exécuté** (binaire k6 absent) ; la cible VPS reste à faire.
...
| P2-2 ✅ (2026-09-20) | **Contrats d'accueil (semaines types, annualisation)** | La facturation des concurrents est générée depuis le contrat + les présences. | **FAIT** — migration 070 : `contracts.weekly_schedule` (jours ISO, défaut DZ dim→jeu, CHECK), `hours_per_day`, `annual_weeks`, `daily_rate`, `absence_deduction`, `extra_day_rate`. Création : montant mensuel lissé = `daily_rate × jours/semaine × annual_weeks ÷ 12` si `monthly_base_amount` absent (422 sinon), **un seul contrat actif par enfant et par période** (409 `CONTRACT_OVERLAP`). Facturation : `contractPresence()` croise la semaine type et `attendance_sessions` du mois → déduction des absences sur jours contractuels (ligne `adjustment` négative, plafonnée au forfait) et supplément des jours présents hors contrat (`extra_day_rate`) ; C04 (`total = subtotal − discount`) respecté, `presence` renvoyée avec la facture. Gate **phase59**. | **L** |
...
| P2-4 ✅ (2026-09-20) | **Pré-inscriptions & liste d'attente** | Pilier d'acquisition ; demandé par les collectivités. | **FAIT** — migration 070 : `enrollment_requests` (RLS forcée, `PRE-<année>-<séq>`, statuts `pending/waitlisted/offered/accepted/declined/withdrawn/expired` avec CHECK de cohérence). Module `enrollment` (director + receptionist) : `POST/GET /enrollment/requests` (file ordonnée par **score explicable** : fratrie +50, enfant du personnel +30, ancienneté +1/semaine ≤ 20, `rank`), `GET /enrollment/capacity/:siteId` (Σ `max_capacity` − inscrits/pré-inscrits − offres en cours), `POST …/waitlist`, `POST …/offer` (409 `SITE_FULL`, 422 salle hors site, expiration paramétrable, **expiration appliquée à la lecture** sans job), `POST …/decide` (acceptation = création de l'enfant `pre_registered` dans la salle offerte, `child_id` figé ; 409 sans offre / offre expirée / déjà décidé). Gate **phase59**. | **M** |
| P2-5 ✅ (2026-09-20) | **Planning du personnel** | Pilier RH ; complète paie et ratios. | **FAIT** — migration 070 : `staff_shifts` (RLS forcée, **EXCLUDE gist** anti-chevauchement par membre — incontournable par l'API, `btree_gist`). `staff-schedule.service.ts` : `POST /staff/schedule/shifts` (409 `SHIFT_OVERLAP`, 422 horaires, 409 si absence déjà déclarée), `DELETE`, `GET /staff/schedule?from&to&site_id`, `POST /staff/schedule/generate-week` (créneaux depuis les affectations actives, semaine DZ, idempotent : `created/skipped`), `GET /staff/schedule/coverage?date` (par salle et par heure 07–18 h : éducateurs planifiés vs effectif attendu — sessions du jour sinon effectif actif — confrontés à **RATIO_EDUC**, `gaps` + `alerts`). Gate **phase59**. | **M-L** |
