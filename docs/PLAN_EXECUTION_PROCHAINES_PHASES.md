...
| v2.11 | **Correctifs d'audit externe** (phase22) : fix P0 paiement (pool brute → withTenantConnection + assertion rowCount → 500 PAYMENT_STATE_ERROR), politique serveur vidéo (préfixe `${tenantId}/video/`, anti-`..`, backend serveur seul, `local` interdit en prod), gardes resolve/containment (API + worker `localPath()`), migration 049 (CHECK storage_key ×4 tables) + 050 (métriques SECURITY DEFINER — jauges étaient à 0 sous NOBYPASSRLS), garde anti-bypass RLS `scripts/check-rls-usage.mjs` (61 accès revus), schema-check renforcé (FORCE/policy/ancrage tenant), eslint réel (0 erreur), 3 tests unitaires jest payment-provider. **Prouvé par mutation** (rouge pré-correctif → vert) | `payment-provider.service`, `video/*`, `exports.service`, worker `pdf.ts`/`exports.ts`, migrations 049-050, `phase22-audit-fixes.api.test.mjs` (41 assertions) | ✅ |
